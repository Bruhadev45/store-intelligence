...
async def _conversion_rate(session, store_id: str, start: datetime, end: datetime) -> float | None:
    ...
    if uv == 0:
        return None
    pos_visitors_q = (
        select(func.distinct(p.visitor_id)).where(
            and_(p.store_id == store_id, p.timestamp >= start, p.timestamp < end)
        )
    )
    pos_visitors = {r[0] for r in (await session.execute(pos_visitors_q)).all() if r[0]}
    billed_q = (
        select(func.distinct(e.visitor_id)).where(
            and_(
                e.store_id == store_id,
                e.timestamp >= start,
                e.timestamp < end,
    ...
    billed = {r[0] for r in (await session.execute(billed_q)).all()}
    converted = billed & pos_visitors

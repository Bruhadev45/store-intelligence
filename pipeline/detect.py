...
Model: RT-DETR-L (Ultralytics build). Filters to class==person.
...
def process_clip(
    *,
    ...
    # Detector: RT-DETR via Ultralytics unless a factory is injected.
    if detector_factory is None:
        detector_factory = _default_rtdetr_factory
    ...
    while True:
        ...
        if entry_line is not None:  # only entry camera tracks billing queue
...
def _default_rtdetr_factory():
    """Lazy-load RT-DETR-L from Ultralytics."""
    from ultralytics import RTDETR  # type: ignore

    model = RTDETR("rtdetr-l.pt")

    def detect(frame) -> list[tuple[tuple[float, float, float, float], float]]:
        results = model.predict(frame, classes=[0], verbose=False, conf=0.20)
